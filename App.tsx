...
const App: React.FC = () => {
    ...
    const handleReadFull = async (chunk: SourceChunk) => {
        ...
        // Если chapter содержит путь (как в случае с CC), пробуем распарсить
        if (typeof chunk.chapter === 'string' && (chunk.chapter.includes('/') || chunk.chapter.includes('\\'))) {
            // Пытаемся извлечь полезную информацию из пути
            // Например из cc\madhya\20\151\index.html
            ...
            if (parts.length >= 2) {
                ...
                if (numbers.length >= 2) {
                    niceChapter = numbers[numbers.length - 2]; // Предпоследнее число - глава
                    niceVerse = numbers[numbers.length - 1];   // Последнее число - стих
                } else if (parts.length >= 3) {
                    // Если это CC, там может быть madhya/20/151
                    if (parts.some(p => ['adi', 'madhya', 'antya'].includes(p.toLowerCase()))) {
                        ...
                        const verseNum = parts.reverse().find(p => /^\d+$/.test(p)); // Ищем с конца
                            ...
                            >
                                ...
                                >
                                    ...
                                    <div className="mt-2 flex items-center justify-between opacity-50 text-[10px] uppercase tracking-wider">
                                        ...
                                        {msg.role === 'model' && (
                                            <span className="flex items-center gap-1"><Sparkles size={10} className="text-amber-400" />SHUKABASE AI Agent</span>
                                        )}
                                    </div>
                                </div>
                            </div>
