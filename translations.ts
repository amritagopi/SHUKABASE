export const TRANSLATIONS = {
    en: {
        appTitle: "SHUKABASE",
        appSubtitle: "AI-powered Assistant for studying the books of His Divine Grace A.C. Bhaktivedanta Swami Srila Prabhupada",
        demoMode: "Demo Mode",
        connected: "Connected",
        settings: "Settings",
        retrievedVerses: "Retrieved Verses",
        found: "found",
        noVerses: "No verses loaded.",
        searchPlaceholder: "Your search results will appear here.",
        manualSearch: "Manual Search",
        search: "Search",
        context: "Context",
        inputPlaceholder: "Ask a question about the scriptures...",
        consulting: "Consulting scriptures...",
        welcomeTitle: "SHUKABASE",
        welcomeText: "Select a conversation or start a new chat.",
        save: "Save",
        dataConnection: "Data Connection",
        backendUrl: "Backend URL",
        bridgeHint: "Start `bridge.py` to enable retrieval from your local FAISS index.",
        agentThinking: "thinking...",
        agentExecuting: "executing",
        agentResult: "result",
        agentWorking: "Agent working...",
        error: "Error",
        newChat: "New Chat",
        history: "History",
        bridge: "Bridge",
        rename: "Rename",
        export: "Export",
        delete: "Delete",
        cancel: "Cancel",
        confirmDelete: "Are you sure you want to delete this chat?",
        chatRenamed: "Chat renamed",
        chatDeleted: "Chat deleted",
        chatExported: "Chat exported",
        quotaExceeded: "Quota exceeded for the current model. Please switch to a different model in Settings and ask the assistant to continue.",
        readFull: "Read Full",
        citation: "ref",
        jumpToSource: "Jump to source",
        rateLimitHint: "Select a different model if you hit rate limits.",
        aiProvider: "AI Provider",
        openRouterDescription: "From time to time OpenRouter provides free Google models. You can also use paid Google models.",
        checkAvailableModels: "available models",
        openRouterApiKey: "OpenRouter API Key",
        keySavedLocally: "Key is saved locally using localStorage.",
        model: "Model",
        refreshList: "Refresh List",
        openRouterFooter: "Showing Google Gemini 2.0+ models with Tool support and pricing.",
        settingsFooterText: "SHUKABASE is my gift to everyone studying shastras and interested in Krishna consciousness.",
        createdWithLove: "Created with love for lecturers, students, and bhakti practitioners. To support the author - click the heart",
        aiDisclaimer: "Artificial Intelligence can make mistakes just like humans - please verify information with authoritative sources.",
        geminiApiKey: "Google Gemini API Key (Required for Knowledge Base)",
        books: {
            "sb": "Srimad-Bhagavatam",
            "bg": "Bhagavad-gita As It Is",
            "cc": "Sri Caitanya-caritamrta",
            "nod": "Nectar of Devotion",
            "noi": "Nectar of Instruction",
            "iso": "Sri Isopanisad",
            "kb": "Krishna Book",
            "tlc": "Teachings of Lord Caitanya",
            "ssr": "Science of Self Realization",
            "tqk": "Teachings of Queen Kunti",
            "pop": "Path of Perfection",
            "poy": "Perfection of Yoga",
            "lcfl": "Life Comes from Life",
            "sc": "Science of Self Realization",
            "rv": "Raja-Vidya",
            "bbd": "Beyond Birth and Death",
            "owk": "On the Way to Krsna",
            "ej": "Easy Journey to Other Planets",
            "lob": "Light of the Bhagavata",
            "spl": "Prabhupada Lilamrta",
            "Uddhava-Gita": "Uddhava Gita",
            "Bhakti-Sandarbha": "Bhakti Sandarbha",
            "Comprehensive-Isopanisad-Notes": "Comprehensive Isopanisad Notes",
            "Sri-Bhakti-sandarbha-12": "Sri Bhakti sandarbha 12",
            "Sri-Bhakti-sandarbha-8": "Sri Bhakti sandarbha 8",
            "Bhakti-Shastri-Student-Handbook": "Bhakti Shastri Student Handbook",
            "Iso": "Isopanisad (Extra)",
            "Sri-Bhakti-sandarbha-2": "Sri Bhakti sandarbha 2",
            "Bhakti-pravesa-Students-Handbook-2012": "Bhakti Pravesa Student's Handbook 2012",
            "bhagvat-subodhini": "Bhagavata Subodhini",
            "Sat-Sandarbha-Class": "Sat Sandarbha Class",
            "Bhagavata-Subodhini-1-2": "Bhagavata Subodhini 1-2",
            "Bhakti-Ratnakara-Narahari-Dasa-I-pdf": "Bhakti Ratnakara",
            "Tattva-Sandarbha-docx": "Tattva Sandarbha",
            "StudyIsopanisad-NOI": "Study Isopanisad & NOI",
            "Complete-Study-Guide-for-Nectar-of-Devotion": "Complete Study Guide for Nectar of Devotion",
            "NOI-nectar-of-instructions-Notes-iskcon": "NOI Notes (ISKCON)",
            "Bhagavata-Vidyalaya-Canto-1-2-3": "Bhagavata Vidyalaya (Canto 1-3)",
            "Bhagavata-Vidyalaya-Canto-7-8-9": "Bhagavata Vidyalaya (Canto 7-9)",
            "Gita-Subodhini-Enriching-the-Experience-of-Bhagavad-Gita-Study": "Gita Subodhini",
            "Caitanya-Subodhini-Gauranga-Darshan-Das-All-Overview-Charts-Unlocked": "Caitanya Subodhini (Overview)",
            "BHAGAVATA-SUBODHINI-4": "Bhagavata Subodhini 4",
            "Subodhini-3": "Subodhini 3",
            "Upadesamrita-Subodhini-GDD": "Upadesamrita Subodhini",
            "Bhagavata-Subodhini-5-6": "Bhagavata Subodhini 5-6",
            "Bhagavata-Subodhini-Canto-7": "Bhagavata Subodhini Canto 7"
        },
        quickQueries: "Quick Queries",
        sendRequest: "Send Request",
        activateProtocol: "Activate Protocol",
        openAiTools: "AI Tools Studio",
        multilingualSearch: "Multilingual Search",
        multilingualSearchHint: "Search across RU and EN databases simultaneously (uses more tokens)",
        responseGenerationFailed: "Response generation failed."
    },
    ru: {
        appTitle: "SHUKABASE",
        appSubtitle: "Ассистент на базе искусственного интеллекта для изучения книг Его Божественной Милости А.Ч. Бхактиведанта Свами Шрилы Прабхупады",
        demoMode: "Демо режим",
        connected: "Подключено",
        settings: "Настройки",
        retrievedVerses: "Найденные стихи",
        found: "найдено",
        noVerses: "Стихи не загружены.",
        searchPlaceholder: "Здесь появятся результаты поиска.",
        manualSearch: "Ручной поиск",
        search: "Поиск",
        context: "Контекст",
        inputPlaceholder: "Задайте вопрос по писаниям...",
        consulting: "Изучаю писания...",
        welcomeTitle: "SHUKABASE",
        welcomeText: "Выберите переписку или начните новый чат.",
        save: "Сохранить",
        dataConnection: "Источник данных",
        backendUrl: "URL бэкенда",
        bridgeHint: "Запустите `bridge.py` для доступа к локальному индексу FAISS.",
        agentThinking: "думаю...",
        agentExecuting: "выполняю",
        agentResult: "результат",
        agentWorking: "Агент работает...",
        error: "Ошибка",
        newChat: "Новый чат",
        history: "История",
        bridge: "Мост",
        rename: "Переименовать",
        export: "Экспорт",
        delete: "Удалить",
        cancel: "Отмена",
        confirmDelete: "Вы уверены, что хотите удалить этот чат?",
        chatRenamed: "Чат переименован",
        chatDeleted: "Чат удален",
        chatExported: "Чат экспортирован",
        quotaExceeded: "Лимиты на текущую модель исчерпаны. Пожалуйста, переключитесь на другую модель в настройках, после чего попросите ассистента продолжить.",
        readFull: "Читать полностью",
        citation: "цитата",
        jumpToSource: "Перейти к источнику",
        rateLimitHint: "Выберите другую модель, если достигнуты лимиты запросов.",
        aiProvider: "AI Провайдер",
        openRouterDescription: "Время от времени в OpenRouter появляются бесплатные модели Google. Также доступны платные модели Google.",
        checkAvailableModels: "доступные модели",
        openRouterApiKey: "Ключ OpenRouter API",
        keySavedLocally: "Ключ сохраняется локально в браузере.",
        model: "Модель",
        refreshList: "Обновить список",
        openRouterFooter: "Показаны модели Google Gemini 2.0+ с поддержкой инструментов и стоимостью.",
        settingsFooterText: "SHUKABASE — мой подарок всем, кто изучает шастры и интересуется сознанием Кришны.",
        createdWithLove: "Создано с любовью для лекторов, учеников и практикующих бхакти. Чтобы поддержать автора - нажмите на сердечко",
        aiDisclaimer: "Искусственный интеллект может ошибаться так же, как и люди — пожалуйста, сверяйте информацию с авторитетными источниками.",
        geminiApiKey: "Ключ Google Gemini API (Обязателен для базы знаний)",
        books: {
            "sb": "Шримад-Бхагаватам",
            "bg": "Бхагавад-гита как она есть",
            "cc": "Шри Чайтанья-чаритамрита",
            "nod": "Нектар преданности",
            "noi": "Нектар наставлений",
            "iso": "Шри Ишопанишад",
            "kb": "Источник вечного наслаждения",
            "tlc": "Учение Господа Чайтаньи",
            "ssr": "Наука самоосознания",
            "tqk": "Учение царицы Кунти",
            "pop": "Путь к совершенству",
            "poy": "Совершенство йоги",
            "lcfl": "Жизнь происходит из жизни",
            "sc": "Наука самоосознания",
            "rv": "Раджа-видья",
            "bbd": "По ту сторону рождения и смерти",
            "owk": "На пути к Кришне",
            "ej": "Легкое путешествие на другие планеты",
            "lob": "Свет Бхагаваты",
            "spl": "Прабхупада-лиламрита",
            "Наука самоосознания": "Наука самоосознания",
            "Учение Шри Чайтаньи": "Учение Шри Чайтаньи",
            "Шри Ишопанишад": "Шри Ишопанишад",
            "Нектар наставлений": "Нектар наставлений",
            "Путешествие вглубь себя": "Путешествие вглубь себя",
            "Совершенные вопросы, совершенные ответы": "Совершенные вопросы, совершенные ответы",
            "Учение Господа Капилы": "Учение Господа Капилы",
            "Молитвы царицы Кунти": "Молитвы царицы Кунти",
            "Раджа-Видья - царь знания": "Раджа-Видья - царь знания"
        },

        quickQueries: "Быстрые запросы",
        sendRequest: "Отправить запрос",
        activateProtocol: "Активировать Протокол",
        openAiTools: "Студия AI Инструментов",
        multilingualSearch: "Мультиязычный поиск",
        multilingualSearchHint: "Искать одновременно в RU и EN базах данных (тратит больше токенов)",
        responseGenerationFailed: "Не удалось сгенерировать ответ."
    }
};
